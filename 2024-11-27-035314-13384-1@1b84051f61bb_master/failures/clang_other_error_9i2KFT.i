...
apm_read (apm_info * i)
{
    ...
    if (fgets (buffer, sizeof (buffer) - 1, str) == ((void*)0))
        printf ("fgets error\n");
...
dev_t apm_dev (void)
{
    ...
    if (access ("/proc/apm", 4) || apm_read (&i) == 1)
        return cached = -1;
    if (i.driver_version[0] == '1')
        return cached = gnu_dev_makedev (10, 134);

    if (!(str = fopen ("/proc/devices", "r")))
        return -1;
    while (fgets (buf, sizeof (buf) - 1, str))
    {
        buf[sizeof (buf) - 1] = '\0';
        for (pt = buf; *pt && ((*__ctype_b_loc ())[(int) ((*pt))] & (unsigned short int) _ISspace); ++pt);
        for (; *pt && !((*__ctype_b_loc ())[(int) ((*pt))] & (unsigned short int) _ISspace); ++pt);
        if (((*__ctype_b_loc ())[(int) ((*pt))] & (unsigned short int) _ISspace))
        {
            *pt++ = '\0';
            pt[strlen (pt) - 1] = '\0';
            if (!strcmp (pt, "apm_bios"))
            {
                fclose (str);
                return cached = gnu_dev_makedev (atoi (buf), 0);
            }
        }
    }
    fclose (str);
...
apm_open (void)
{
    ...
    if (i.driver_version[0] >= '1')
    {
        if ((fd = open ("/dev/apm_bios", 02)) < 0)
        {



            if (mknod ("/dev/apm_bios", 0020000 | 0400 | 0200, apm_dev ()))
            {
                unlink ("/dev/apm_bios");
                return -1;
            }
            fd = open ("/dev/apm_bios", 02);
        }
        return fd;
    }
    return -1;
...
apm_get_events (int fd, int timeout, apm_event_t * events, int n)
{
    ...
    do { unsigned int __i; fd_set *__arr = (&fds); for (__i = 0; __i < sizeof (fd_set) / sizeof (__fd_mask); ++__i) ((__arr)->__fds_bits)[__i] = 0; } while (0);
    ((void) (((&fds)->__fds_bits)[((fd) / (8 * (int) sizeof (__fd_mask)))] |= ((__fd_mask) (1UL << ((fd) % (8 * (int) sizeof (__fd_mask)))))));
    retcode = select (fd + 1, &fds, ((void*)0), ((void*)0), timeout < 0 ? ((void*)0) : &t);
    if (retcode <= 0)
        return 0;
...
{
    sync ();
    return ioctl (fd, (((0U) << (((0 +8)+8)+14)) | ((('A')) << (0 +8)) | (((2)) << 0) | ((0) << ((0 +8)+8))), ((void*)0));
}
...
{
    sync ();
    return ioctl (fd, (((0U) << (((0 +8)+8)+14)) | ((('A')) << (0 +8)) | (((1)) << 0) | ((0) << ((0 +8)+8))), ((void*)0));
}
...
static const lookup_t error_table[] = {
    ...
    { 0x02, "Real mode interface already connected" },
    { 0x03, "Interface not connected" },
    { 0x05, "16 bit interface already connected" },

    { 0x07, "32 bit interface already connected" },
    { 0x08, "32 bit interface not supported" },
    { 0x09, "Unrecognized device ID" },
    { 0x0a, "Parameter out of range" },
...
apm_error_name (unsigned int err)
{
  ...
  for (i=0; i<(sizeof(error_table)/sizeof(lookup_t)); i++)
