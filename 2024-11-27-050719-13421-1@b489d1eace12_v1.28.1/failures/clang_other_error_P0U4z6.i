...
__attribute__((visibility("hidden"))) void autoscaler_init (AutoScaler *that, gint64 interval, guint64 floor);
__attribute__((visibility("hidden"))) guint64 autoscaler_get_max (AutoScaler *that, guint64 current);
# 4 "autoscaler.c" 2
                    ...
                    guint64 current)
{
    ...
    that->max = (((that->max) > (current)) ? (that->max) : (current));
    that->max = (((that->max) > (that->floor)) ? (that->max) : (that->floor));
